...
class ReceiveFileDialog extends ReceiveDialog {
    ...
    async _displayFiles(peerId, displayName, connectionHash, files, imagesOnly, totalSize, badgeClassName) {
        ...
        document.title = files.length === 1
            ? `${ Localization.getTranslation("document-titles.file-received") } - Drop`
            : `${ Localization.getTranslation("document-titles.file-received-plural", null, {count: files.length}) } - Drop`;
...
class ReceiveRequestDialog extends ReceiveDialog {
    ...
    _showRequestDialog(request, peerId) {
        ...
        document.title =  `${transferRequestTitle} - Drop`;
        changeFavicon("images/favicon-96x96-notification.png");
...
class PairDeviceDialog extends Dialog {
    ...
    _getPairUrl() {
        ...
        url.searchParams.set('pair_key', this.pairKey);
        return url.href;
    }
...
class ReceiveTextDialog extends Dialog {
    ...
    _setDocumentTitleMessages() {
        document.title = this._receiveTextQueue.length <= 1
            ? `${ Localization.getTranslation("document-titles.message-received") } - Drop`
            : `${ Localization.getTranslation("document-titles.message-received-plural", null, {count: this._receiveTextQueue.length + 1}) } - Drop`;
    }
